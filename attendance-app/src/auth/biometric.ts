/** WebAuthn + stored Identity accessKey — mirrors Flutter biometric login. */

const STORAGE_KEY = 'bisync_rms_web_biometric'
...
export function loadBiometricEnrollment(): BiometricEnrollment | null {
  try {
    const raw = localStorage.getItem(STORAGE_KEY)
    if (!raw) return null
    const data = JSON.parse(raw) as BiometricEnrollment
    if (!data?.username || !data?.accessKey || !data?.credentialId) return null
    return data
  } catch {
    return null
  }
...
export function saveBiometricEnrollment(enrollment: BiometricEnrollment) {
...
/** Prompt for device biometrics before using the stored access key. */
