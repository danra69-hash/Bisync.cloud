/**
 * Device-local 4-digit PIN unlock — encrypts a refresh token (or session snapshot)
 * so the user can sign in without typing their password.
 */

const STORAGE_KEY = 'bisync_rms_web_pin'

export type PinPayload =
  | { kind: 'refresh'; refreshToken: string; username: string }
  | {
      kind: 'session'
      username: string
      accessToken: string
...
export function loadPinEnrollment(): { username: string } | null {
  try {
    const raw = localStorage.getItem(STORAGE_KEY)
    if (!raw) return null
    const data = JSON.parse(raw) as PinEnrollmentStored
    if (data?.version !== 1 || !data.username || !data.ciphertext) return null
...
export function isPinEnabled(username?: string | null) {
  const enrolled = loadPinEnrollment()
  if (!enrolled) return false
  if (
    username &&
    enrolled.username.toLowerCase() !== username.trim().toLowerCase()
  ) {
    return false
  }
  return true
...
export async function savePinEnrollment(pin: string, payload: PinPayload) {
...
export async function unlockPinPayload(pin: string): Promise<PinPayload> {
  if (!isValidPin(pin)) throw new Error('PIN must be exactly 4 digits')
  const raw = localStorage.getItem(STORAGE_KEY)
  ...
  try {
    ...
    const payload = JSON.parse(new TextDecoder().decode(plain)) as PinPayload
    if (!payload?.username || !payload.kind) {
