/**
 * Device-local 4-digit PIN unlock for Team portal.
 * Encrypts a small session snapshot so staff can sign in without the password.
 */

const STORAGE_KEY = 'bisync-team-pin';

export type TeamPinPayload = {
  kind: 'team-session';
  username: string;
  employeeId: number;
  name: string;
...
export async function savePinEnrollment(pin: string, payload: TeamPinPayload) {
...
export async function unlockPinPayload(pin: string): Promise<TeamPinPayload> {
  if (!isValidPin(pin)) throw new Error('PIN must be exactly 4 digits');
  const raw = localStorage.getItem(STORAGE_KEY);
  ...
  try {
    ...
    const payload = JSON.parse(new TextDecoder().decode(plain)) as TeamPinPayload;
    if (!payload?.username || payload.kind !== 'team-session' || !payload.employeeId) {
