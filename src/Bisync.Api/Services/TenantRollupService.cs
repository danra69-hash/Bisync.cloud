    ...
    ILogger<TenantRollupService> logger)
{
    ...
    async Task<TenantRollupResult> BuildFanOutAsync(CancellationToken ct)
    {
        ...
        locationRows = locationRows
            .OrderBy(l => l.CompanyName, StringComparer.OrdinalIgnoreCase)
            .ThenBy(l => l.LocationName, StringComparer.OrdinalIgnoreCase)
            .ToList();
    ...
    static string SubscriptionKey(int companyId, string locationExternalId) =>
        $"{companyId}|{(locationExternalId ?? string.Empty).Trim()}";
