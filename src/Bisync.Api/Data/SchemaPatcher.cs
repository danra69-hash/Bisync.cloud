...
public static class SchemaPatcher
{
    public static async Task ApplyAsync(BisyncDbContext db)
    {
        ...
        await EnsureIntegrationOutboxTableAsync(db);
    ...
    static async Task EnsureTenantConnectionsTableAsync(BisyncDbContext db)
    {
        await db.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS "TenantConnections" (
                ...
                "PlacementMode" TEXT NOT NULL DEFAULT 'shared',
        ...
        await DatabaseSchemaHelper.EnsureColumnAsync(db, "TenantConnections", "PlacementMode", "TEXT NOT NULL DEFAULT 'shared'");
        await DatabaseSchemaHelper.EnsureColumnAsync(db, "TenantConnections", "ShardId", "INTEGER NOT NULL DEFAULT 0");
        await db.Database.ExecuteSqlRawAsync("""
            CREATE UNIQUE INDEX IF NOT EXISTS "IX_TenantConnections_CompanyId"
            ON "TenantConnections" ("CompanyId");
            """);
        await db.Database.ExecuteSqlRawAsync("""
            CREATE INDEX IF NOT EXISTS "IX_TenantConnections_PlacementMode"
            ON "TenantConnections" ("PlacementMode");
            """);
        await db.Database.ExecuteSqlRawAsync("""
        ...
        await db.Database.ExecuteSqlRawAsync("""
            UPDATE "TenantConnections"
            SET "PlacementMode" = CASE
                WHEN "ConnectionString" IS NOT NULL AND BTRIM("ConnectionString") <> '' THEN 'dedicated'
                ELSE 'shared'
            END
            WHERE "PlacementMode" IS NULL OR BTRIM("PlacementMode") = '';
            """);
    }

    static async Task EnsureIntegrationOutboxTableAsync(BisyncDbContext db)
    {
        await db.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS "IntegrationOutbox" (
                ...
                "CreatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
                "AvailableAt" TIMESTAMP WITH TIME ZONE NULL,
                "ProcessedAt" TIMESTAMP WITH TIME ZONE NULL
            );
            """);
        await db.Database.ExecuteSqlRawAsync("""
            CREATE INDEX IF NOT EXISTS "IX_IntegrationOutbox_Status_AvailableAt_Id"
            ON "IntegrationOutbox" ("Status", "AvailableAt", "Id");
            """);
        await db.Database.ExecuteSqlRawAsync("""
            CREATE INDEX IF NOT EXISTS "IX_IntegrationOutbox_CompanyId_CreatedAt"
            ON "IntegrationOutbox" ("CompanyId", "CreatedAt");
            """);
    }
