...
public static class SchemaPatcher
{
    public static async Task ApplyAsync(BisyncDbContext db)
    {
        ...
        await DatabaseSchemaHelper.TryAddColumnAsync(db, "Companies", "TimeZoneId", "TEXT NOT NULL DEFAULT ''");
        ...
        await DatabaseSchemaHelper.TryAddColumnAsync(db, "B2bSalesOrders", "ReservedOnly", "BOOLEAN NOT NULL DEFAULT FALSE");
        await TryCreateIndexAsync(db, "IX_B2bSalesOrders_ShareToken", "B2bSalesOrders", "\"ShareToken\"");
        await BackfillCompanyTimeZonesAsync(db);
        await db.Database.ExecuteSqlRawAsync("""
    ...
    static async Task BackfillCompanyTimeZonesAsync(BisyncDbContext db)
    {
        var companies = await db.Companies
            .Where(c => c.TimeZoneId == null || c.TimeZoneId == "")
            .ToListAsync();
        ...
        // Align location clocks to the company operating timezone when empty.
        var locations = await db.Locations
            .Where(l => l.TimeZoneId == null || l.TimeZoneId == "")
            .ToListAsync();
        if (locations.Count == 0)
            return;

        var companyById = await db.Companies.AsNoTracking()
            .ToDictionaryAsync(c => c.Id);
        foreach (var location in locations)
        {
            Company? company = null;
            if (location.CompanyId is int locCompanyId)
                companyById.TryGetValue(locCompanyId, out company);
            OrgClock.AssignLocationTimeZone(location, company);
        }
