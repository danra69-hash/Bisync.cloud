...
public static class DataSeeder
{
    public static async Task SeedAsync(BisyncDbContext db)
    {
        ...
        db.Ingredients.AddRange(
            new Ingredient { ComponentId = "CMP-WAGYUB-001", Name = "Wagyu Beef A5", Category = "Food", Group = "Proteins", RecipeUom = "g", LastPriceRecipe = 0.38m, DailyUsage = 2.4m, OrderFreqDays = 3, StorageJson = JsonSerializer.Serialize(new[] { "Freezer" }), DetailConfigJson = """{"altRecipeUnits":[{"fromQty":"1","qty":"1000","unit":"kg"}]}""", AttachedProducts = 3, AttachedVendors = 2, Active = true, LocationsJson = JsonSerializer.Serialize(new[] { "downtown", "midtown" }) },
            new Ingredient { ComponentId = "CMP-BLACKT-001", Name = "Black Truffle", Category = "Food", Group = "Produce", RecipeUom = "g", LastPriceRecipe = 1.80m, DailyUsage = 45m, OrderFreqDays = 7, StorageJson = JsonSerializer.Serialize(new[] { "Chiller" }), AttachedProducts = 2, AttachedVendors = 1, Active = true, LocationsJson = JsonSerializer.Serialize(new[] { "downtown" }) },
            new Ingredient { ComponentId = "CMP-BURRAT-001", Name = "Burrata", Category = "Food", Group = "Dairy", RecipeUom = "pcs", LastPriceRecipe = 8.75m, DailyUsage = 8m, OrderFreqDays = 2, StorageJson = JsonSerializer.Serialize(new[] { "Chiller" }), AttachedProducts = 4, AttachedVendors = 1, Active = true, LocationsJson = JsonSerializer.Serialize(new[] { "all" }) }
        );
